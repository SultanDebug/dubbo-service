 ...
 */
@Slf4j
public class MyTableShardingIntegerAlgorithm implements PreciseShardingAlgorithm<Integer>{
    @Override
    public String doSharding(Collection<String> collection, PreciseShardingValue<Integer> preciseShardingValue) {
        ...
        if ("user".equalsIgnoreCase(preciseShardingValue.getLogicTableName())) {
            for (String each : collection) {
                try {
                    //分表算法，+1表示表名下标从1开始
                    if (each.endsWith((preciseShardingValue.getValue() % collection.size()+1) + "")) {
                        log.info("table:{}", each);
                        tableName = each;
                        break;
                    }
                }catch (Exception e){
                    log.error("位置异常:{}",e.getMessage());
                }
            }
        }
